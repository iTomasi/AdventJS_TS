...
const countPackages = (carriers: any[], carrierID: string) => {
    const findMain = carriers.find((value) => value[0] === carrierID);

    if (!findMain) return 0

    let quantity: number = 0;

    quantity += findMain[1];

    for (let value of findMain[2]) {
        const find = carriers.find((id) => id[0] === value)

        if (!find) continue

        quantity += find[1]

        if (find[2][0] !== undefined) {
            for (let value_1 of find[2]) {
                const subsCarriersQuantity: number = countPackages(carriers, value_1);

                quantity += subsCarriersQuantity
            }
        }
    }
