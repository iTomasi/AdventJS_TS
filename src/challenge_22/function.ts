...
const countDecorations = (bigTree: any) => {
    let count: number = !bigTree.value ? 0 : bigTree.value

    if (bigTree.left) {
        const leftCount = countDecorations(bigTree.left)
        count += leftCount
    }

    if (bigTree.right) {
        const rightCount = countDecorations(bigTree.right);
        count += rightCount
    }
