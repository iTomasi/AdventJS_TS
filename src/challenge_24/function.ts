...
const checkIsSameTree = (treeA: any, treeB: any): boolean => {

    if (treeA.value !== treeB.value) return false
    else if (treeA.left === treeB.left || treeA.right === treeB.right) return true

    let checkingTree: boolean = false;

    if (treeA.left && treeB.left) {
        const sameTree: boolean = checkIsSameTree(treeA.left, treeB.left);

        checkingTree = sameTree
    }

    if (treeA.right && treeB.right) {
        const sameTree: boolean = checkIsSameTree(treeA.right, treeB.right);
